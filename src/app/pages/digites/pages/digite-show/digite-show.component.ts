...
@Component({
  ...
  templateUrl: './digite-show.component.html',
  styleUrls: ['./digite-show.component.scss']
})
export class DigiteShowComponent implements OnInit {
  data = signal<Digite | null>(null);
  ...
  loadData(id: string): void {
    ...
    this.genericApi.getOne<Digite>('digites', id).subscribe({
      next: (res) => {
        const response = res.data as Digite;
        if (response) {
  ...
  back(): void {
    this.router.navigate(['index/digites']);
  }

  edit(): void {
    if (this.identifier) {
      this.router.navigate(['index/digites/edit', this.identifier]);
    }
  }
