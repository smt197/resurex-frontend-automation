...
@Component({
  ...
  templateUrl: './outil-show.component.html',
  styleUrls: ['./outil-show.component.scss']
})
export class OutilShowComponent implements OnInit {
  data = signal<Outil | null>(null);
  ...
  loadData(id: string): void {
    ...
    this.genericApi.getOne<Outil>('outils', id).subscribe({
      next: (res) => {
        const response = res.data as Outil;
        if (response) {
  ...
  back(): void {
    this.router.navigate(['index/outils']);
  }

  edit(): void {
    if (this.identifier) {
      this.router.navigate(['index/outils/edit', this.identifier]);
    }
  }
