...
@Component({
  selector: 'vex-outil-create-update',
  standalone: true,
  imports: [
    CommonModule,
    MatCardModule,
  ...
  templateUrl: './outil-create-update.component.html',
  styleUrls: ['./outil-create-update.component.scss']
})
export class OutilCreateUpdateComponent implements OnInit {
  form = new FormGroup({});
  model: any = {};
  ...
  ngOnInit(): void {
    ...
    this.fields = outilsFormlyFields.map(field => {
  ...
  loadData(): void {
    this.isLoading = true;
    this.genericApi.getOne<Outil>('outils', this.id!.toString()).subscribe({
      next: (response) => {
        this.model = response.data;
        this.isLoading = false;
      },
      error: (error) => {
        console.error('Error loading outil:', error);
  ...
  save(): void {
    if (this.form.valid) {
      ...
      const config = OUTILS_CONFIG;
      const useFormData = config.data.useFormData;

      if (this.mode === 'create') {
        const createObservable = useFormData
          ? this.genericApi.create<Outil>('outils', this.formDataBuilder.createFormData(this.model))
          : this.genericApi.createJson<Outil>('outils', this.model);

        createObservable.subscribe({
          next: (response) => {
            this.snackBar.open(
              response!.message || 'Outil created successfully!',
              'OK',
              { duration: 3000 }
            );
          ...
          error: (error) => {
            console.error('Error creating outil:', error);
            this.snackBar.open('Error creating outil', 'OK', { duration: 3000 });
            this.isLoading = false;
          }
        });
      } else {
        const updateObservable = useFormData
          ? this.genericApi.update<Outil>('outils', this.id!.toString(), this.formDataBuilder.createFormData(this.model))
          : this.genericApi.updateJson<Outil>('outils', this.id!.toString(), this.model);

        updateObservable.subscribe({
          next: (response) => {
            this.snackBar.open(
              response.message || 'Outil updated successfully!',
              'OK',
              { duration: 3000 }
            );
            this.router.navigate(['../..'], { relativeTo: this.route });
          ...
          error: (error) => {
            console.error('Error updating outil:', error);
            this.snackBar.open('Error updating outil', 'OK', { duration: 3000 });
            this.isLoading = false;
          }
        });
      }
