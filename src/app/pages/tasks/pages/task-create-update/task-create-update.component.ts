...
@Component({
  selector: 'vex-task-create-update',
  standalone: true,
  imports: [
    CommonModule,
    MatCardModule,
...
export class TaskCreateUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.fields = tasksFormlyFields.map(field => {
  ...
  loadData(): void {
    this.isLoading = true;
    this.genericApi.getOne<Task>('tasks', this.id!.toString()).subscribe({
      next: (response) => {
  ...
  save(): void {
    if (this.form.valid) {
      ...
      const config = TASKS_CONFIG;
      const useFormData = config.data.useFormData;

      if (this.mode === 'create') {
        const createObservable = useFormData
          ? this.genericApi.create<Task>('tasks', this.formDataBuilder.createFormData(this.model))
          : this.genericApi.createJson<Task>('tasks', this.model);

        createObservable.subscribe({
          next: (response) => {
            this.snackBar.open(
              response!.message || 'Tâche created successfully!',
              'OK',
              { duration: 3000 }
            );
          ...
          error: (error) => {
            ...
            this.snackBar.open('Error creating task', 'OK', { duration: 3000 });
            this.isLoading = false;
          }
        });
      } else {
        const updateObservable = useFormData
          ? this.genericApi.update<Task>('tasks', this.id!.toString(), this.formDataBuilder.createFormData(this.model))
          : this.genericApi.updateJson<Task>('tasks', this.id!.toString(), this.model);

        updateObservable.subscribe({
          next: (response) => {
            this.snackBar.open(
              response.message || 'Tâche updated successfully!',
              'OK',
              { duration: 3000 }
            );
            this.router.navigate(['../..'], { relativeTo: this.route });
          ...
          error: (error) => {
            ...
            this.snackBar.open('Error updating task', 'OK', { duration: 3000 });
            this.isLoading = false;
          }
        });
      }
