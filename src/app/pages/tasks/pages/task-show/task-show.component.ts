...
export class TaskShowComponent implements OnInit {
  ...
  loadData(id: string): void {
    ...
    this.genericApi.getOne<Task>('tasks', id).subscribe({
      next: (res) => {
        const response = res.data as Task;
        if (response) {
  ...
  edit(): void {
    if (this.identifier) {
      this.router.navigate(['index/tasks/edit', this.identifier]);
    }
  }
